...
class BaseModel(nn.Module):
    def __init__(self, args, dataset):
        ...
        self.kg_aug = args.kg_aug
        self.kg_aug_dim = args.kg_aug_dim
        self.group_aug = args.group_aug
        if self.group_aug:
            self.augment_num += 3
        args.augment_num = self.augment_num
        self.with_aug = args.augment or args.group_aug or args.kg_aug
        ...
        if self.task == 'ctr':
            ...
            if self.augment_num:
                if self.args.augment:
                    orig_dens_vec.append(inp['hist_aug_vec'].to(device))
                    orig_dens_vec.append(inp['item_aug_vec'].to(device))
                if self.group_aug:
                    orig_dens_vec.append(inp['item_group_aug_vec'].to(device))
                    orig_dens_vec.append(inp['user_pos_group_aug_vec'].to(device))
                    orig_dens_vec.append(inp['user_neg_group_aug_vec'].to(device))
                dens_vec = self.convert_module(orig_dens_vec)

            if self.kg_aug:
                kg_item_vec = inp['kg_aug_vec'].to(device)
                kg_vec = self.kg_convert_module(kg_item_vec)
                if dens_vec is not None:
                    dens_vec = torch.concat([dens_vec, kg_vec], dim=-1)
                    orig_dens_vec.append(kg_item_vec)
                else:
    ...
    def get_field_num(self):
        if self.kg_aug:
            return self.item_fnum + self.augment_num + self.hist_fnum + 1
        else:
...
class DenseLR(BaseModel):
    def __init__(self, args, dataset):
        super(DenseLR, self).__init__(args, dataset)
        self.final_fc = nn.Linear(self.dense_dim * self.augment_num, 1)

    def forward(self, inp):
        _, _, _, _, orig_dens_vec, labels = self.process_input(inp)
        dense_ft = torch.cat(orig_dens_vec, dim=-1)
        logits = self.final_fc(dense_ft)
        out = self.get_ctr_output(logits, labels)
        return out


class DenseMLP(BaseModel):
    def __init__(self, args, dataset):
        super(DenseMLP, self).__init__(args, dataset)
        self.final_mlp = MLP(self.final_mlp_arch, self.dense_dim * self.augment_num, self.dropout)
        self.final_fc = nn.Linear(self.final_mlp_arch[-1], 1)

    def forward(self, inp):
        _, _, _, _, orig_dens_vec, labels = self.process_input(inp)
        dense_ft = torch.cat(orig_dens_vec, dim=-1)
        mlp_out = self.final_mlp(dense_ft)
...
class SetRank(BaseModel):
    def __init__(self, args, dataset):
        ...
        self.layerNorm1 = nn.LayerNorm(self.module_inp_dim)
        self.attention = nn.MultiheadAttention(self.module_inp_dim, args.n_head, batch_first=True,
                                               dropout=args.attn_dp)
        self.layerNorm2 = nn.LayerNorm(self.module_inp_dim * 2)
        self.mlp = MLP(args.final_mlp_arch, self.module_inp_dim * 2, self.dropout)
        self.fc_out = nn.Linear(args.final_mlp_arch[-1], 1)

    def forward(self, inp):
        item_embed, labels = self.process_rerank_inp(inp)
        item_embed = self.layerNorm1(item_embed)
        attn_out, _ = self.attention(item_embed, item_embed, item_embed)
        mlp_in = torch.cat([attn_out, item_embed], dim=-1)
        mlp_in = self.layerNorm2(mlp_in)
        mlp_out = self.mlp(mlp_in)

        scores = self.fc_out(mlp_out).view(-1, self.max_list_len)
        scores = torch.sigmoid(scores)
        outputs = self.get_rerank_output(scores, labels, attn=False)
        return outputs
