...
class MoE(nn.Module):
    ...
    def __init__(self, moe_arch, inp_dim, dropout):
        ...
        self.gate_net = nn.Linear(inp_dim, expert_num)
        self.expert_net = nn.ModuleList([MLP(expert_arch, inp_dim, dropout) for _ in range(expert_num)])

    def forward(self, x):
        gate = self.gate_net(x).view(-1, self.expert_num)  # (bs, expert_num)
        gate = nn.functional.softmax(gate, dim=-1).unsqueeze(dim=1) # (bs, 1, expert_num)
        experts = [net(x) for net in self.expert_net]
...
class HEA(nn.Module):
    ...
    def forward(self, x_list):
        ...
        gates = torch.stack(gates, dim=1)  # (bs, tower_num, expert_num), expert_num = share_expt_num + spcf_expt_num
        ...
        expert_mix = torch.matmul(gates, experts).squeeze(dim=2)  # (bs, tower_num, E)
        # print('expert mix', expert_mix.shape, 'tower num', self.tower_num)
        expert_mix = torch.split(expert_mix, dim=1, split_size_or_sections=1)
        out = [x.squeeze(dim=1) for x in expert_mix]
        return out
...
class ConvertNet(nn.Module):
    ...
    def __init__(self, args, inp_dim, dropout, conv_type):
        ...
        print(self.type)
        if self.type == 'MoE':
            moe_arch = args.expert_num, args.convert_arch
            self.sub_module = MoE(moe_arch, inp_dim, dropout)
            print('convert module: MoE', moe_arch)
        elif self.type == 'HEA':
            ple_arch = args.expert_num, args.specific_expert_num, args.convert_arch, args.augment_num
            self.sub_module = HEA(ple_arch, inp_dim, dropout)
            print('convert module: HEA', ple_arch)
        else:
            self.sub_module = MLP(args.convert_arch, inp_dim, dropout).to(self.device)
            print('convert module: MLP', args.covert_arch)
